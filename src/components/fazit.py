...
def _render_user_fazit(df_orig, df_long, user_name):
    ...
    daily = (
        df_orig.groupby('date')['total_minutes']
        .first()
        .sort_index()
    )

    if daily.empty:
        items.append(("ℹ️", "Für den gewählten Zeitraum liegen keine Daten vor.", "neutral"))
        _render_items(items)
        return
    ...
    if df_long is not None and not df_long.empty:
        app_totals = df_long.groupby('App')['Minutes'].sum().sort_values(ascending=False)
        if not app_totals.empty:
            top_app = app_totals.index[0]
            top_app_min = app_totals.iloc[0]
            share = (top_app_min / app_totals.sum()) * 100 if app_totals.sum() > 0 else 0
            items.append((
                "📱",
...
def _render_team_fazit(df_orig, df_long):
    ...
    if 'User' not in df_orig.columns:
        items.append(("ℹ️", "Keine Nutzerinformation in den Daten gefunden.", "neutral"))
        _render_items(items)
        return

    # Pro Nutzer und Tag aggregieren
    per_user_day = (
        df_orig.groupby(['User', 'date'])['total_minutes']
        .first()
        .reset_index()
    )
    ...
    user_total = per_user_day.groupby('User')['total_minutes'].sum().sort_values(ascending=False)
    ...
    if df_long is not None and not df_long.empty and 'User' in df_long.columns:
        # Top-App pro Nutzer
        app_per_user = (
            df_long.groupby(['User', 'App'])['Minutes'].sum()
            .reset_index()
        )
    ...
    LIMIT = 180
    under_per_user = (
        per_user_day[per_user_day['total_minutes'] < LIMIT]
        .groupby('User').size()
    )
